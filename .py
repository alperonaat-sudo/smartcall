...
# ---------------- Masaüstü klasörü ve ayarlar dosyası ----------------
...
# ================== Ayar Dosyası (dosya işlemleri) ==================
def save_settings(data):
    """Not ayarlarını Invekto klasörüne kaydet"""
    try:
...
def load_settings_file():
    """Invekto klasöründen ayarları yükle"""
...
class ExcelAyirici:
    def __init__(self, parent):
        ...
        self.window.title("Excel Ayırma")
        self.window.geometry("480x500")

        ctk.CTkButton(self.window, text="📂 Excel Seç", command=self.choose_excel).pack(pady=10, fill="x", padx=20)
        ...
        ctk.CTkButton(self.window, text="✅ Kaydet", command=self.save_filtered_excel, fg_color="green").pack(pady=10, fill="x", padx=20)
        ctk.CTkButton(self.window, text="❌ Kapat", command=self.window.destroy, fg_color="red").pack(pady=5, fill="x", padx=20)
    ...
    def save_filtered_excel(self):
        ...
        if not self.secili_durumlar:
            messagebox.showwarning("Uyarı", "En az bir durum seçin veya manuel girin!")
            return
        ...
        new_wb.close()
        messagebox.showinfo("Tamamlandı", f"{len(self.secili_durumlar)} durum seçildi ve yeni Excel kaydedildi!")

# ================== Ana Bot ==================
class InvektoBot(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("📞 Invekto Smart Call")
        self.geometry("360x600")

        # Excel / arama değişkenleri
        ...
        # Not ayarları ve UI değişkenleri
        ...
        # numaralara ait geçici notlar
        self.numara_durumlari: dict[str, str] = {}

        # UI / pencere referansları
        self.durum_entries = {}
        self.not_win = None
        self.current_num = None
        self.unused_win = None  # yeni pencere referansı
    ...
    def _build_ui(self):
        ...
        ctk.CTkButton(frame_top, text="📂 Excel Seç", command=self.load_excel).pack(pady=5, fill="x")
        ctk.CTkButton(frame_top, text="🎯 Arama Kutusu Kordinat Seç", command=self.set_coord).pack(pady=5, fill="x")
        ctk.CTkButton(frame_top, text="❌ Çağrıyı Bitir Kordinat Seç", command=self.set_kapat_coord,
                      fg_color="#B71C1C", text_color="white", hover_color="#7F0000").pack(pady=5, fill="x")

        self.btn_start = ctk.CTkButton(frame_top, text="▶ Başlat", command=self.toggle_bot, fg_color="green")
        self.btn_start.pack(pady=5, fill="x")

        frame_mod = ctk.CTkFrame(self)
        frame_mod.pack(pady=5, fill="x", padx=10)
        #ctk.CTkSwitch(frame_mod, text="", variable=self.otomatik_mod,  # BU KISIM SES İŞLEME İLE BİRLİKTE DEVREYE ALINACAKTIR
        ...
        ctk.CTkLabel(frame_hotkey, text="Başlat/Durdur Tuşu:").pack(side="left", padx=5)
        self.entry_hotkey = ctk.CTkEntry(frame_hotkey, placeholder_text="örn: F8 veya z", width=120)
        ...
        ctk.CTkButton(frame_durum_btn, text="📝 Not Ayarları", command=self.open_not_ayar).pack(pady=5, fill="x")

        frame_bottom = ctk.CTkFrame(self)
        frame_bottom.pack(pady=5, fill="x", padx=10)
        ctk.CTkButton(frame_bottom, text="💾 Excel’i Kaydet", command=self.save_excel, fg_color="blue").pack(
            side="left", expand=True, fill="x", padx=5, pady=5)
        ctk.CTkButton(frame_bottom, text="💾 Kaydet & Çık", command=self.on_closing, fg_color="red").pack(
            side="left", expand=True, fill="x", padx=5, pady=5)

        frame_extra = ctk.CTkFrame(self)
        frame_extra.pack(side="bottom", fill="x", padx=10, pady=10)
        ctk.CTkButton(frame_extra, text="📊 Excel Ayır", command=lambda: ExcelAyirici(self),
    ...
    def save_excel(self):
        if not self.excel_path:
            messagebox.showwarning("Uyarı", "Kaydedilecek bir Excel dosyası yok!")
            return
        try:
            wb = openpyxl.load_workbook(self.excel_path)
            ws = wb.active
            ...
            for idx, num in self.numbers:
                ...
                if durum:  # sadece durum varsa güncelle
            ...
            messagebox.showinfo(
                "Kaydedildi",
                f"Notlar '{os.path.basename(self.excel_path)}' dosyasına kaydedildi."
            )
        except Exception as e:
            messagebox.showerror("Kaydetme Hatası", f"Excel kaydedilemedi:\n{e}")
    ...
    def safe_exit(self):
        try:
            ...
            print("Kapanış sırasında hata:", e)
        finally:
            try:
                os._exit(0)
            except Exception:
    ...
    # -------------------- Ayar Kaydet / Yükle ( NOT AYARLARI) -------------------
    def save_settings(self):
        """
        Sadece 'Not Ayarları' kalıcı olacak:
        - durum_tanimlari (anahtarlar string olarak yazılır)
        - not_al (bool)
        - show_not_window (bool)
        - hotkey_str (string)
        NOT: numara_durumlari veya excel verileri JSON'a yazılmaz.
        """
        try:
            data = {
    ...
    def load_settings(self):
        """
        Yükleme: JSON'dan durum_tanimlari alınır (string anahtarlar -> int anahtarlar).
        Ayrıca not_al, show_not_window ve hotkey_str geri yüklenir.
        """
        data = load_settings_file()
        if not data:
            return

        # durum_tanimlari: anahtarları int'e çevir
        ...
        try:
            ...
            def set_start():
                try:
                    ...
                    top.destroy()
                    messagebox.showinfo(
                        "Excel Yüklendi", f"{len(self.numbers)} numara bulundu.\nBaşlangıç satırı: {start_val}"
                    )
                    self.update_sayac()
                except Exception:
            ...
            top = Toplevel(self)
            top.title("Başlangıç Satırı")
            top.geometry("400x180")

            lbl = ctk.CTkLabel(top, text="Kaçıncı sıradan başlansın?", text_color="black")
            lbl.pack(pady=15)
    ...
    def set_coord(self):
        messagebox.showinfo("Koordinat", "Arama kutusuna fareyi götür ve 3 sn bekle...")
    ...
    def set_kapat_coord(self):
        messagebox.showinfo("Koordinat", "Kapat butonuna fareyi götür ve 3 sn bekle...")
    ...
    def set_hotkey(self):
        ...
        if not key:
            messagebox.showerror("Hata", "Lütfen bir tuş girin! (örn: F8 veya z)")
            return
        try:
            if key.startswith("f") and key[1:].isdigit():
                ...
                self.hotkey = key
            self.hotkey_str = key
            messagebox.showinfo("Kısayol Ayarlandı", f"Başlat/Durdur için yeni tuş: {key.upper()}")
            # değişikliği kaydet (not ayarlarını kaydeT,)
            try:
                self.save_settings()
            except Exception:
                pass
        except Exception as e:
            messagebox.showerror("Hata", f"Kısayol atanamadı: {e}")
    ...
    def toggle_bot(self):
        if self.running:
            ...
            self.btn_start.configure(text="▶ Başlat", fg_color="green")
        else:
            if not self.numbers:
            ...
            self.btn_start.configure(text="⏹ Durdur", fg_color="red")
    ...
    def run_bot(self):
        ...
        self.running = False
        self.ui_call(self.btn_start.configure, text="▶ Başlat", fg_color="green")

    # -------------------- Not Ayarları --------------------
    def open_not_ayar(self):
        win = Toplevel(self)
        win.title("Not Ayarları")
        win.geometry("420x570")

        ctk.CTkSwitch(win, text="📝 Not Al", variable=self.not_al, text_color="black").pack(pady=5)
        ctk.CTkSwitch(win, text="Not Penceresi Göster", variable=self.show_not_window, text_color="black").pack(
            pady=5
        )
        ...
        def kaydet():
            ...
            # sadece NOT AYARLARINI kaydet (numara notlarını değil)
            self.save_settings()
            win.destroy()

        ctk.CTkButton(win, text="Kaydet", command=kaydet).pack(pady=10)

    # -------------------- Not Al (numara için geçici pencere) --------------------
    ...
    def show_not_al_window(self, num: str):
        ...
        lbl = ctk.CTkLabel(
            self.not_win,
            text=f"{num}\nNumaranın durumunu seç veya yaz:",
            justify="left",
            text_color="black",
        )
        ...
        def kaydet_durum(durum: str):
            # bu notu sadece geçici olarak saklıyoruz ve Excel'e yazıyoruz,
            # fakat numara_durumlari JSON içine kaydedilmeyecek (kullanıcı istediği gibi)
            self.numara_durumlari[num] = durum
            if self.excel_path:
                try:
                    row_idx = self._find_row_for_number(num)
                    if row_idx is None:
                        raise ValueError("Numara excel listesinde bulunamadı.")
                    wb = openpyxl.load_workbook(self.excel_path)
                    ws = wb.active
    ...
    def on_key_press(self, key):
        try:
            # ESC tuşu → botu her durumda durdur
            if key == pyn_keyboard.Key.esc:
                if self.running:
                    self.ui_call(self.toggle_bot)
                return

            # Eğer not penceresi açıksa ve hotkey'e basılırsa kapat
            if self.not_win and getattr(self.not_win, "winfo_exists", lambda: False)():
                matches = False
            ...
            # Hotkey entry'si aktifken görmezden gel
    ...
    def on_closing(self):
        ...
        # Excel kaydı (tüm notları aktarma)
        if self.excel_path:
            try:
                wb = openpyxl.load_workbook(self.excel_path)
                ws = wb.active
                durum_col = 3  # C sütunu
                for idx, num in self.numbers:
                    durum = self.numara_durumlari.get(num, "")
                    if durum:
                        ws.cell(row=idx, column=durum_col, value=durum)
                ...
                print("Excel kaydetme hatası on_closing:", e)
